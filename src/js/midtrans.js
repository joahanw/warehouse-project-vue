...
const MAX_SNAP_INIT_RETRIES = 10;

class MidtransService {
  constructor() {
    this.config = MIDTRANS_CONFIG;
    this.snap = null;
    this.watchScriptLoadFailure();
    this.initSnap();
  }

  /**
   * Listen for the snap.js <script> tag failing to load (blocked by
   * network/firewall/adblocker) so we get an explicit error instead of
   * silently polling forever.
   */
  watchScriptLoadFailure() {
    ...
    if (scriptEl) {
      scriptEl.addEventListener("error", () => {
        console.error(
          "❌ Failed to load Midtrans snap.js. Check network connectivity to app.sandbox.midtrans.com (firewall/adblocker may be blocking it).",
        );
      });
    }
  }
  ...
  initSnap(attempt = 0) {
    ...
    if (attempt >= MAX_SNAP_INIT_RETRIES) {
      console.error(
        `❌ Midtrans Snap failed to initialize after ${MAX_SNAP_INIT_RETRIES} attempts. window.snap is still unavailable — the snap.js script likely never loaded.`,
      );
      return;
    }
...
export const handlePaymentCallback = (result) =>
  midtransService.handlePaymentCallback(result);
