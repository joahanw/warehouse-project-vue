  ...
  >
      ...
      >
        <div id="Header" class="flex items-center justify-between px-[18px]">
          <div class="flex flex-col gap-[6px]">
            ...
            >
              ⚠️
              {{ availableProducts.length - getAvailableProductsCount() }}
        ...
        <div class="modal-header flex items-center justify-between">
          <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-monday-gray">
              {{ getAvailableProductsCount() }} dari
          ...
          <div class="flex flex-col gap-5 w-full">
            ...
            >
              ...
              <p class="font-semibold text-red-600">
                Semua produk merchant ini stoknya habis.
              </p>
              <p class="text-monday-gray text-center">
                Silakan update stok produk terlebih dahulu sebelum melakukan
                transaksi.
              </p>
            </div>
            ...
            >
              <div class="flex items-center justify-between gap-6">
                ...
                >
                  ...
                  <p
                    class="amount min-w-12 font-medium text-[22px] text-center"
                  >
              ...
              <div class="flex items-center justify-between">
                <p
                  class="flex items-center gap-1 font-semibold text-monday-gray"
                >
                  <img
                    src="@/assets/images/icons/money-grey.svg"
                    class="size-6 flex shrink-0"
    ...
    >
      ...
      >
        ...
        <div
          class="modal-content flex flex-1 flex-col items-center gap-6 overflow-y-auto pb-4"
        >
          ...
          <div v-if="!scannedProduct" class="text-center">
            ...
            <p class="font-semibold text-lg text-monday-blue mb-2">
              Posisikan barcode dalam kotak
            </p>
            <p class="text-monday-gray">
              Scanner akan otomatis mendeteksi barcode produk
            </p>
            <div class="mt-3 p-3 bg-blue-50 rounded-lg">
              ...
              <ul class="text-xs text-blue-700 text-left space-y-1">
                ...
                <li>
                  • Untuk barcode "SWP-0012345", gunakan tombol test di bawah
                </li>
              </ul>
            </div>
          ...
          <div v-if="scanHistory.length > 0" class="w-full max-w-[500px]">
            <div class="bg-gray-50 rounded-2xl p-4">
              <div class="flex items-center justify-between mb-3">
                <p class="font-semibold text-sm text-monday-gray">
          ...
          >
            ...
            <!-- Barcode Info -->
            <div
              class="mb-3 p-2 bg-green-50 border border-green-200 rounded-lg"
            >
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="flex flex-col gap-2 flex-1">
                ...
                <p class="font-semibold text-monday-blue">
            ...
            <!-- Product details -->
            <div
              v-if="scannedProduct.product_about"
          ...
          >
            <p class="font-semibold text-monday-red text-center">
              {{ scanError }}
            </p>
          </div>
        </div>
  ...
  try {
    ...
    if (response.data) {
      ...
      availableProducts.value = products.map((product) => ({
        ...
        price: `Rp ${formatPrice(product.product.price)}`,
        stock: product.stock,
        category: product.product.category?.name,
        categoryIcon: product.product.category?.photo,
        image: product.product.thumbnail,
        selectedQuantity: 0,
        // Additional data from API
        product_about: product.product.about,
        warehouse_name: product.warehouse.name,
        warehouse_phone: product.warehouse.photo,
        // Add barcode field for scanning
        barcode: product.product.barcode || product.product.id.toString(),
      }));
    } else {
    ...
    Quagga.init(
      ...
      (err) => {
        if (err) {
          ...
          scanError.value =
            "Gagal memulai scanner. Silakan gunakan input manual.";
          scannerStatus.value = "error";
          return;
        }
...
const searchProductByBarcode = async (barcode) => {
  ...
  try {
    ...
    const response = await getMerchantProductByBarcode(
      searchCode,
      customerInfo.value.merchantId,
    );
    if (response.data) {
      ...
      const product = {
        ...
        price: `Rp ${formatPrice(productData.product.price)}`,
        stock: productData.stock,
        category: productData.product.category?.name,
        categoryIcon: productData.product.category?.photo,
        image: productData.product.thumbnail,
        // Additional data from API
        product_about: productData.product.about,
        warehouse_name: productData.warehouse.name,
        warehouse_phone: productData.warehouse.phone,
        merchant_id: productData.merchant.id,
      ...
      try {
        localStorage.setItem(
          ...
          JSON.stringify(scanHistory.value),
        );
      } catch (error) {
...
const addScannedProduct = () => {
  ...
  const existingProduct = assignedProducts.value.find(
    (p) => p.id === scannedProduct.value.id,
  );
  if (existingProduct) {
    if (existingProduct.quantity + 1 > scannedProduct.value.stock) {
      alert(
        `❌ Tidak bisa menambahkan lebih banyak. Stok tersedia: ${scannedProduct.value.stock}`,
      );
      return;
    }

    transactionStore.updateProductQuantity(
      existingProduct.id,
...
const addToCart = () => {
  ...
  if (selectedProducts.length === 0) {
    alert(
      "Tidak ada produk yang dipilih atau semua produk yang dipilih stoknya habis",
    );
    return;
  }
  ...
  const invalidProducts = selectedProducts.filter(
    (p) => p.selectedQuantity > p.stock,
  );
  if (invalidProducts.length > 0) {
    const productNames = invalidProducts.map((p) => p.name).join(", ");
    alert(
      `❌ Kuantitas yang dipilih melebihi stok tersedia untuk produk: ${productNames}`,
    );
    return;
  }
