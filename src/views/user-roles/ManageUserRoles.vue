<template>
  <Layout pageTitle="Manage User Role">
    <main class="flex flex-col gap-6 flex-1">
      ...
      >
        <div id="Header" class="flex items-center justify-between px-[18px]">
          ...
          <a href="/add-user-roles" class="btn btn-primary font-semibold">
            Add New
            <img
        ...
        <div id="Product-List" class="flex flex-col px-4 gap-5 flex-1">
          ...
          >
            <img
              ...
              alt="loading"
            />
            <p class="font-medium text-monday-gray mt-4">
              Loading user roles data...
            </p>
          </div>
                ...
                <div class="flex items-center gap-3">
                  ...
                  >
                    <img
                      :src="
                        userRole.user.photo ||
                        '@/assets/images/photos/photos-2.png'
                      "
                ...
                <div class="flex items-center gap-3 w-[226px] shrink-0">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <p class="font-semibold text-lg text-nowrap">
                      {{ userRole.role.map((obj) => obj.name).join(", ") }}
                    </p>
                  </div>
                </div>
                <router-link
                  :to="`/edit-user-roles/${userRole.user.id}/details/${userRole.role.id}`"
    ...
    const fetchUserRoles = async (page = 1) => {
      ...
      try {
        const params = new URLSearchParams({
          ...
          pageSize: limit.value.toString(),
        });

        const response = await getUserRoles(`?${params.toString()}`);
        userRoles.value = response.data?.content;
        currentPage.value = response.data.page;
        totalPages.value = response.data.totalPages;
        totalRecords.value = response.data.totalElements;
        limit.value = response.data.size;
