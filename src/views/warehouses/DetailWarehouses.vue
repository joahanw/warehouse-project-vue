<template>
  <Layout
    ...
    breadcrumb-text="Detail Warehouse"
  ...
  <div
    ...
    class="modal hidden flex flex-1 items-center justify-center h-full fixed top w-full p-4"
  >
    <div
      data-close-modal
      class="backdrop absolute w-full h-full bg-[#292D32B2]"
    ...
    >
      ...
      >
        ...
        <div>
          <p class="font-medium text-sm text-monday-gray">Product About</p>
          <p class="font-semibold leading-[160%]">
            A printer is an essential device for printing documents, photos, and
            various other needs. There are several common types of printers,
            such as inkjet, laser, and thermal
          </p>
        </div>
      </div>
    </div>
  ...
  methods: {
    async loadWarehouseProducts() {
      ...
      try {
        ...
        this.warehouse = {
          id: response.data[0].warehouse.id,
          name: response.data[0].warehouse.name,
          phone: response.data[0].warehouse.phone,
          photo: response.data[0].warehouse.photo,
        };

        if (response.data[0].product !== null) {
          this.warehouseProducts = response.data.map((item) => ({
            ...item.product,
            stock: item.stock,
          }));
        }
