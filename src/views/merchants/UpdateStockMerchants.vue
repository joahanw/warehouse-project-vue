<template>
  <Layout
    pageTitle="Update Stock Merchant"
    breadcrumb-text="Update Stock Merchant"
  ...
  methods: {
    async loadDetailsFromQuery() {
      ...
      this.productDetails = {
        ...
        thumbnail: query.product_thumbnail || "",
      };

      this.form.stock = this.productDetails.currentStock;
    },
    ...
    async handleSubmit() {
      ...
      const productId = this.$route.params.productId;
      const merchantId = this.$route.params.merchantId;
      const merchantProductId = this.$route.query.merchant_product_id;
      try {
        // Prepare request data using data from query parameters
        const requestData = {
          stock: parseInt(this.form.stock),
          warehouseId: parseInt(this.$route.query.warehouse_id) || null,
        ...
        const response = await updateMerchantProductStock(
          merchantProductId,
          requestData,
        );
