          ...
          <div class="flex flex-col gap-[6px]">
            <p class="flex items-center gap-[6px]">
              ...
              <span class="font-semibold text-2xl"
                >{{ categories.length }} Total Category</span
              >
            </p>
...
export default {
  ...
  methods: {
    ...
    getCategoryStatus(category) {
      if (category.countProduct > 0) {
        return {
          status: "protected",
          message: `Protected - ${this.formatNumber(category.countProduct)} products`,
          canDelete: false,
        };
      }
      return {
    ...
    async deleteCategory(categoryId, categoryName) {
      ...
      if (
        confirm(`Apakah Anda yakin ingin menghapus kategori "${categoryName}"?`)
      ) {
        try {
